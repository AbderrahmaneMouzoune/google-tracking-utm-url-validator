...
export const columns: ColumnDef<UrlWithError>[] = [
  {
    ...
    header: 'Valid',
    cell: ({ row }) => {
      const missingParameters: PossibleParameters[] =
        row.getValue('missingParameters')
      return (
        <div
          className={cn('size-4 rounded-full mx-auto', {
      ...
      return (
        <div className="flex gap-1">
          {missingParameters.map((param) => (
            <Badge key={`${url}-${param}`} variant={'destructive'}>
              {param}
            </Badge>
          ))}
        </div>
